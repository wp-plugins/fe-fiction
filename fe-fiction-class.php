...
function FeFiction_Init_Options()
{
	...
	if(get_option($fe_fiction_wp_options['custom_dashboard']) == '1')
	{
		...
		{
			add_action('admin_notices', 'new_cms_dashboard_widgets_pages' );
		}
	}
	...
	if(get_option($fe_fiction_wp_options['enable_fe_fiction_default_role']) != '')
	{
		if(is_multisite())
		{
			update_site_option( 'default_user_role', $fe_fiction_default_role );
		}
		else
		{
			update_option('default_role',$fe_fiction_default_role);
		}
	}
...
function FeFiction_Insert_Rewrite_Rules($wp_rewrite)
{
	...
	$new_rules = array( 
		/**
		$custom_post_type.'/[^/]+/attachment/([^/]+)/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1)
		,$custom_post_type.'/[^/]+/attachment/([^/]+)/trackback/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1).'&tb=1'
		,$custom_post_type.'/[^/]+/attachment/([^/]+)/feed/(feed|rdf|rss|rss2|atom)/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1).'&feed='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/[^/]+/attachment/([^/]+)/(feed|rdf|rss|rss2|atom)/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1).'&feed='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/[^/]+/attachment/([^/]+)/comment-page-([0-9]{1,})/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1).'&cpage='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/([^/]+)/trackback/?$'=>'index.php?pagename='.$pagename.'&fiction='.$wp_rewrite->preg_index(1).'&tb=1'
		,$custom_post_type.'/([^/]+)/feed/(feed|rdf|rss|rss2|atom)/?$'=>'index.php?pagename='.$pagename.'&fiction='.$wp_rewrite->preg_index(1).'&feed='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/([^/]+)/(feed|rdf|rss|rss2|atom)/?$'=>'index.php?pagename='.$pagename.'&fiction='.$wp_rewrite->preg_index(1).'&feed='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/([^/]+)/page/?([0-9]{1,})/?$'=>'index.php?pagename='.$pagename.'&fiction='.$wp_rewrite->preg_index(1).'&paged='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/([^/]+)/comment-page-([0-9]{1,})/?$'=>'index.php?pagename='.$pagename.'&fiction='.$wp_rewrite->preg_index(1).'&cpage='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/([^/]+)(/[0-9]+)?/?$'=>'index.php?pagename='.$pagename.'&fiction='.$wp_rewrite->preg_index(1).'&page='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/[^/]+/([^/]+)/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1)
		,$custom_post_type.'/[^/]+/([^/]+)/trackback/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1).'&tb=1'
		,$custom_post_type.'/[^/]+/([^/]+)/feed/(feed|rdf|rss|rss2|atom)/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1).'&feed='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/[^/]+/([^/]+)/(feed|rdf|rss|rss2|atom)/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1).'&feed='.$wp_rewrite->preg_index(2)
		,$custom_post_type.'/[^/]+/([^/]+)/comment-page-([0-9]{1,})/?$'=>'index.php?pagename='.$pagename.'&attachment='.$wp_rewrite->preg_index(1).'&cpage='.$wp_rewrite->preg_index(2)
		,**/
...
{
	global $custom_post_type,$fe_fiction_wp_options;

	$fe_fiction_page_page_id = get_option($fe_fiction_wp_options['fe_fiction_page_page_id'],'0');
	$permalink = get_permalink( $fe_fiction_page_page_id );
	$pagename = str_replace('/','',str_replace(home_url().'/','',$permalink));

	return str_replace($custom_post_type,$pagename.'/read_story',$location);
}
...
function new_cms_dashboard_widget_pages_function() {
	/** from fe-fiction-class **/
	global $custom_post_type;
    $siteurl = get_option('siteurl');

	/** WordPress Administration Bootstrap */
	include_once('views/new_dashboard_page_simple.php');
} 
...
function new_cms_dashboard_widgets_pages() {
	echo '<div style="padding-top:50px;"></div>';new_cms_dashboard_widget_pages_function();
} 
...
function fe_fiction_restrict_admin_menus()
{
...
function create_fe_fiction_page($page_title)
{
	global $user_ID;
	...
	$new_page = array(
		'filter' => 'db'
		,'post_type' => 'page'
		,'post_title' => stripslashes($page_title)
		,'post_content' => stripslashes('[fe-fiction]')
		,'post_excerpt' => ''
		,'post_category' => array('')
		,'post_author' => $user_ID
		,'tags_input' => ''
...
function FeFiction_Admin_Options_Page()
{
	...
	if(isset($_POST) && is_array($_POST) && isset($_POST['Submit']))
	{
		...
		if(isset($_POST['enable_fe_fiction_default_role']))
		{
			...
			update_option($fe_fiction_wp_options['enable_fe_fiction_default_role'],$fe_fiction_default_role);
			if(is_multisite())
			{
				update_site_option( 'default_user_role', $fe_fiction_default_role );
			}
		...
		/** this is not used right now, but we will set it **/
